...
class Client extends ClientBase
{
  constructor ({ appId, appSecret, refreshToken, token, encodingAESKey,
                 mchId, mchKey, certPath, keyPath, accessToken }) {
    super();
    this.appId = appId;
  ...
  async ticket (type = 'jsapi') {
    if (this._ticket[type]) {
      ...
      let ticket;
      if (Client.redis)
        ticket = await Client.redis.get(this.appId + '_ticket_' + type);

      if (!ticket) {
        let data = await this.getTicket(type);

        if (Client.redis)
          await Client.redis.set(this.appId + '_ticket_' + type, data.ticket, 'EX', data.expiresIn - 600);
